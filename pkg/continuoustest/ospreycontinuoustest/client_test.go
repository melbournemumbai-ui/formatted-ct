...
func TestOTLPHttpClient_WriteSeries(t *testing.T) {
	...
	cfg := continuoustest.ClientConfig{}
	...
	c, err := continuoustest.NewClient(cfg, log.NewNopLogger())
	require.NoError(t, err)
	...
	t.Run("write series in a single batch", func(t *testing.T) {
		...
		series := continuoustest.GenerateSineWaveSeries("test", now, 10)
	...
	t.Run("write series in multiple batches", func(t *testing.T) {
		...
		series := continuoustest.GenerateSineWaveSeries("test", now, 22)
		statusCode, err := c.WriteSeries(ctx, series)
		require.NoError(t, err)
		assert.Equal(t, 200, statusCode)
	...
	t.Run("request failed with 4xx error", func(t *testing.T) {
		...
		series := continuoustest.GenerateSineWaveSeries("test", now, 1)
		statusCode, err := c.WriteSeries(ctx, series)
		require.Error(t, err)
...
func TestClient_QueryRange(t *testing.T) {
	...
	t.Run("results cache disabled", func(t *testing.T) {
		...
		_, err := c.QueryRange(ctx, "up", time.Unix(0, 0), time.Unix(1000, 0), 10, continuoustest.WithResultsCacheEnabled(false))
		require.NoError(t, err)
...
func TestClient_Query(t *testing.T) {
	...
	// c, err := NewOspreyClient(cfg, log.NewNopLogger())
	c, err := continuoustest.NewClient(cfg, log.NewNopLogger())
	require.NoError(t, err)
	...
	t.Run("results cache disabled", func(t *testing.T) {
		...
		_, err := c.Query(ctx, "up", time.Unix(0, 0), continuoustest.WithResultsCacheEnabled(false))
		require.NoError(t, err)
...
func TestClient_QueryHeaders(t *testing.T) {
	...
	testCases := map[string]struct {
		cfgMutator func(*continuoustest.ClientConfig)
		...
		"tenant header is not used when basic auth is used": {
			cfgMutator: func(cfg *continuoustest.ClientConfig) {
